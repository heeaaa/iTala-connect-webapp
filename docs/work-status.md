...
Latest run: **36239532684 on `8919912` (5d rules and images): 207/207 pgTAP, 22/22 integration, 70 E2E with 69 passing first time and 1 flaky** (the Phase 3b guard test, passed on retry; a timing bug in the test, fixed after that run, see the 5d section). Unit coverage, lint, typecheck, build, `check:secrets` and gitleaks passed. Before it: 36234713687 on `167c8e7` (5c-3) was 68/68 E2E with 180 pgTAP. Check the PR's latest run after each push (`gh pr checks 1`). The Docker PC is no longer required for routine verification.
...
- **Flaky guard test: cause and fix (test timing, not a 5d regression):**
  ...
  - Fix: the test now waits for the New event page (its Create event button) before Forward, in both places, as the harness version of this test already did. CI's next run is the check. The race cannot be run as a CI red test, because CI needs the real backend, so the harness diagnostic is the before and after evidence.
