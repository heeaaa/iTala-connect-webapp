-- A save is atomic and never replaces score, provenance or mobile-link rows.
-- Published schedule editing belongs to phase 5; this RPC edits drafts only.
create function public.save_draft_editor(p_event_id uuid, p_version timestamptz, p_details jsonb, p_divisions jsonb)
...
begin
  ...
  if current_event.status <> 'draft' then raise exception 'Only drafts can be saved here' using errcode = '22023'; end if;
  if current_event.updated_at is distinct from p_version then raise exception 'Event changed. Reload before saving.' using errcode = '40001'; end if;
  ...
  if nullif(btrim(p_details->>'name'), '') is null then raise exception 'Event name required' using errcode = '22023'; end if;
  update public.events set name = p_details->>'name',
    ...
    courts = (p_details->>'courts')::smallint, court_names = array(select jsonb_array_elements_text(p_details->'court_names')),
    timezone = p_details->>'timezone', theme_primary = p_details->>'theme_primary', theme_bg = p_details->>'theme_bg',
    theme_text = p_details->>'theme_text', theme_text_secondary = p_details->>'theme_text_secondary', theme_heading = p_details->>'theme_heading'
  ...
  for d in select value from jsonb_array_elements(p_divisions) loop
    ...
    update public.divisions set name=d->>'name', color=d->>'color', bracket_count=(d->>'bracket_count')::smallint,
      custom_games_per_team=(d->>'custom_games_per_team')::boolean, games_per_team=(d->>'games_per_team')::smallint, sort_order=di
    ...
    -- A draft should not contain fixtures. Refuse to orphan any if data was imported manually.
    if exists(select 1 from public.games where event_id=p_event_id and
      (team1_id in (select id from public.teams where division_id=(d->>'id')::uuid and not(id=any(team_ids)))
       or team2_id in (select id from public.teams where division_id=(d->>'id')::uuid and not(id=any(team_ids))))) then
      raise exception 'Remove scheduled games before removing their teams' using errcode='23514';
    end if;
    delete from public.teams where division_id=(d->>'id')::uuid and not(id=any(team_ids));
    di := di + 1;
  end loop;
  if exists(select 1 from public.games where event_id=p_event_id and division_id is not null and not(division_id=any(division_ids))) then
    raise exception 'Remove scheduled games before removing their divisions' using errcode='23514';
  end if;
  delete from public.divisions where event_id=p_event_id and not(id=any(division_ids));
  return next_version;
end; $$;
revoke all on function public.save_draft_editor(uuid,timestamptz,jsonb,jsonb) from public, anon;
grant execute on function public.save_draft_editor(uuid,timestamptz,jsonb,jsonb) to authenticated;
